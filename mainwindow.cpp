    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->SizeInput->setValidator(new QRegularExpressionValidator(expr));
}
    ...
    for(int i = 0; i < n; i++)
        for(int j = 0; j < n; j++)
        {
            ...
            connect(button, &LightButton::clicked, this, [&](){
                ...
                for(int t = 0; t < 5; t++)
                {
                    int ii = dynamic_cast<LightButton*>(QObject::sender())->geti() + di[t];
                    int jj = dynamic_cast<LightButton*>(QObject::sender())->getj() + dj[t];
